...
constructor(
  ...
  private suspend fun onAccountCreated(
    ...
    taskResult.fold(
      { failed -> failed.left() },
      { success ->
        storeUserOnFirestore(credentials, success.uid).fold({ it.left() }, { success.right() })
      }
    )
  ...
  private suspend fun storeUserOnFirestore(
    ...
    if (user.profilePicture != null) {
      val storeResult =
        storeUserProfilePictureToFirebaseStorage(Uri.parse(user.profilePicture), signedUserId)
  ...
  private suspend fun storeUserProfilePictureToFirebaseStorage(
    ...
    return try {
      firebaseStorageAPI
        .uploadFile(NetworkRequest.of(FirebaseStorageRequest(file, "$signedUserId/profile")))
        .data
        ?.await()!!
        .right()
    } catch (e: Exception) {
