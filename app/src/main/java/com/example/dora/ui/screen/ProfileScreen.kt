...
internal fun ProfileForm(
  ...
  OutlinedTextField(
    ...
    visualTransformation =
      if (passwordConfirmationHidden) PasswordVisualTransformation() else VisualTransformation.None,
    trailingIcon = {
      IconButton(onClick = { passwordConfirmationHidden = !passwordConfirmationHidden }) {
        ...
        val description = if (passwordConfirmationHidden) "Show password" else "Hide password"
  ...
  Button(
    ...
    onClick = {
      ...
      val passwordVerificationResult =
        BCrypt.verifyer().verify(passwordConfirmation.toByteArray(), user.password?.toByteArray())
      ...
      scope.launch {
        ...
        if (imageUri.toString() != user.profilePicture) {
          uri = profileViewModel.updateProfilePicture(user.uid!!, imageUri).getOrElse { Uri.EMPTY }
        }
    ...
    Text("Update profile", style = MaterialTheme.typography.bodyLarge, fontWeight = FontWeight.Bold)
  }
}
